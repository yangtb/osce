layui.config({
    ...
    $('#addSection').on('click', function () {
        ...
        common.openOffset('新增目录', basePath + '/pf/p/td/item/section/form?idItemStore=' + idItemStore,
            350, 190, x, y + 35);
    });
    ...
    $('#delSection').on('click', function () {
        ...
        var url = basePath + '/pf/r/td/item/section/del';
    ...
    var _addOrEdit = function (formType, currentEditData) {
        var idItemSection = $('#idItemSection option:selected').val();
        if (formType == 'add') {
            var index = common.open('新增题目', basePath + '/pf/p/td/item/detail/form?formType='
                + formType + '&idItemStore=' + idItemStore + '&idItemSection=' + idItemSection, 500, 350);
            layer.full(index)
        } else {
            var index = common.open('编辑题目', basePath + '/pf/p/td/item/detail/form?formType='
    ...
    var _delItem = function (currentData) {
        var url = basePath + '/pf/r/td/item/exam/del';
        var reqData = new Array();
...
var _setSection = function () {
    layui.use(['jquery', 'form', 'table', 'common'],function(){
        ...
        layer.load(2);
        $.ajax({
            url: basePath + '/pf/r/td/item/section/list',
            type: 'post',
            dataType: 'json',
