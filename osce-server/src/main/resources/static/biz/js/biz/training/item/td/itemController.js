layui.config({
    ...
    table.render({
        ...
        , cols: [[
            ...
            {field: 'naItemStore', width: 170, title: '题集', fixed: true},
        ...
        , url: basePath + '/pf/p/td/item/list'
        , limit: 15
        , even: true
        , limits: [15, 30, 100]
        , page: true
    ...
    var _addOrEdit = function (formType, currentEditData) {
        if (formType == 'add') {
            var index = common.open('新增题集', basePath + '/pf/p/td/item/form?formType=' + formType, 500, 350);
            layer.full(index)
        } else {
            var index = common.open('编辑题集', basePath + '/pf/p/td/item/form?formType=' + formType, 500, 350, _successFunction(currentEditData));
            layer.full(index)
        }
    };
    ...
    var _delGrade = function (currentData) {
        var url = basePath + '/pf/r/td/item/del';
    ...
    form.on('switch(fgActiveCheckFilter)', function (obj) {
        ...
        common.commonPost(basePath + '/pf/r/td/item/updateStatus', data, '设置', obj.othis);
    });
    ...
    function bachImportItem(data) {
        ...
        for (var key in data) {
            ...
            for (var key1 in dataList) {
                $.each(dataList[key1], function (index, content) {
                    if (index > 1) {
                        content.idItemStore = idItemStore;
        ...
        $.ajax({
            url: basePath + '/pf/r/td/item/batch/add',
            type: 'post',
            dataType: 'json',
            contentType: "application/json",
            data: JSON.stringify(reqData),
