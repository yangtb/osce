layui.config({
    ...
    element.on('tab(tagTabFilter)', function (data) {
        if (data.index == 1) {
            if (!$("#itemTag").attr("src")) {
                $('#itemTag').attr('src', basePath + '/pf/p/td/item/manage/page?idItemStore=' + $('#idItemStore').val());
            }
        }
    });
                    ...
                    if (!$('#idItemStore').val()) {
                       element.tabAdd('tagTabFilter', {
                           ...
                           , content: '<iframe id="itemTag" class=\'layui-col-xs12\' frameborder="0" ' +
                               'src= "' + basePath + '/pf/p/td/item/manage/page?idItemStore=' + data.data + '"></iframe>'
                           , id: 'itemTag'
                       });
                       FrameWH();
                    }
